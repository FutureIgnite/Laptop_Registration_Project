...
int register_student (Entries* entry) {
   ...
   strlen(entry->student.student_name) > 5) {
   ...
   if (scanf("%19s", entry->student.reg_no) == 1 && strlen(entry->student.reg_no) > 15) {
   ...
   if ((scanf ("%14s", entry->student.phone_no)) == 1 && strlen(entry->student.phone_no) < 15) {
      flush_input_buff ();
   ...
   if((scanf ("%5s", entry->student.y_of_study)) == 1) {
      flush_input_buff ();
...
int register_laptop (laptop_entries* Laptop) {
