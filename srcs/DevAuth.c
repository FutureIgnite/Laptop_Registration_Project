...
/* Clear the window and draw a box */
...
int main () {
    ...
    char res = 'Y', opt; /* response decision option */
    char reg_no[20];
    Entry *stdnt = NULL;
    WINDOW *child;
    ...
    if (db_init ()) {
        ...
        /* R; -- R is not standard, removed */
        exit (EXIT_FAILURE);
    }

    printw ("Database initialized successfully\n");
    /* R; -- R is not standard, removed */
        ...
        switch (opt) {
            case 'R':
                do {
                    ...
                    wprintw (child, "\t\tPress [B] to go back to the main menu | Press [Enter] to continue: ");
                    wrefresh (child);
                    res = wgetch (child);
                    if (res == 'B') {
            ...
            case 'A':
                ...
                do  {
                    ...
                    if (!(stdnt = lookup_db (reg_no))){
                        wprintw (child, "\t\tStudent does not exist\n");
                        wrefresh (child);
                        goto label;
                    } else {
                        display (stdnt, child);
                    }
                    wprintw (child, "\n\t\tDoes the serial provided match in system ? (Y/N): ");
                    wrefresh (child);
...
label:
                    wprintw (child, "\n\t\tPress [B] to go back to the main menu | Press [Enter] to register: ");
                    wrefresh (child);
                    res = toupper (wgetch (child));
                    if (res == 'B') {
                        break;
